...
void FNNAME(launch)(std::vector<double> &timings, const unsigned int isize,
  ...
  for (unsigned int t = 0; t < tsteps; t++) {
    ...
    gpuErrchk(cudaDeviceSynchronize());
    t1 = std::chrono::high_resolution_clock::now();
    FNNAME(avgi)<<<num_blocks, block_dim>>>(a, b, init_offset, jstride, kstride,
                                            ksize, isize, jsize);
    // gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    ...
    gpuErrchk(cudaDeviceSynchronize());
    t1 = std::chrono::high_resolution_clock::now();
    FNNAME(avgj)<<<num_blocks, block_dim>>>(a, b, init_offset, jstride, kstride,
                                            ksize, isize, jsize);
    // gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    ...
    gpuErrchk(cudaDeviceSynchronize());
    t1 = std::chrono::high_resolution_clock::now();
    FNNAME(avgk)<<<num_blocks, block_dim>>>(a, b, init_offset, jstride, kstride,
                                            ksize, isize, jsize);
    // gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
    ...
    gpuErrchk(cudaDeviceSynchronize());
    t1 = std::chrono::high_resolution_clock::now();
    FNNAME(lap)<<<num_blocks, block_dim>>>(a, b, init_offset, jstride, kstride,
                                           ksize, isize, jsize);
    // gpuErrchk(cudaPeekAtLastError());
    gpuErrchk(cudaDeviceSynchronize());
